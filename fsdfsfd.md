...
in their ability to use interest tax shields. Finally, the securities of these firms —particularly high yield debt and private equity —are likely to be substantially less liquid
...
we use values firms by discounting capital cash flows atthe discount rate for an allequity firm.2 Capital cash flows equal the after-corporate-tax cash flows to both debt
...
+ Depreciation
+ Amortization
+ Change in deferred taxes - Change in net working capital
+ interest (cash and non-cash) - Capital expenditures
...
+ After-tax asset sales
= Capital Cash Flows
In our analysis, we prefer to use the first, net-income-based capital cash flowmeasure over the EBfl'-based measure. The net-income-based measure utilizes
...
r
is the risk premium required by investors to invest in a firm or project with the same
...
To obtain the market asset beta, we calculate the leverage of non-fmancial and nonutility firms in the S&P 500 using the defmition from (4). The median leverage ratio
...
test in section 4 and explicitly consider in section 5. We also use the long-termTreasury bond (approximatelY 20 years to maturity) yield to measure the risk-free rate
...
However, we obtain qualitatively similar results when we base our analyses on ashortterm investment horizon. For a short-term horizon, we estimate the risk-free rate asthe
long-term Treasury bond yield less the historic arithmetic average spread between
Treasury bond and Treasury bill returns, and we use a risk premium equal to the longterm arithmetic average return spread between the S&P 500 and Treasury bills.
...
a set of guideline or comparable firms. Value is estimated by multiplying the ratio fromthe guideline companies by the performance measure for the company being valued.
Valuation by comparables or multiples relies on two assumptions. First, the
comparable companies have proportional future cash flow expectations and risks as the
...
because it incorporates contemporaneous market expectations of future cash flows and
...
is no obvious method to determine which measure of performance — EBITDA, cashflow, net income, and so on — is the appropriate basis of comparison. Consistent with
...
and, therefore, the sample size for this measure is lower. Because the sample fromwhich we draw the comparables includes a large fraction of the HLT universe, we do
...
industry-based asset beta of 0.88, which, in turn, is smaller than the median marketbased asset beta of 0.93. A higher beta increases the discount rate which in turn
...
-18.1%. This is well outside the range of median errors for the Compressed APVmethods.
The comparable transaction based estimates are more accurate, with a median
error of 5.9%, which is in the range of median errors for the Compressed APVestimates. In fact, the mean valuation error of 0.3% for the comparable transactions is
...
The estimates using the finn-based Compressed APV method are within 15%of transaction value for almost one-half of the sample. The industry-based and the
...
This comparable method therefore has about the same percentage of errors within 15%as the Compressed APV methods.
...
industry- and transaction-based comparables. The MAEs of the industry- and marketbased APV methods are significantly smaller than the MAE of the comparable company
18
method.
...
ways. First, if the estimated value exceeds the transaction value and the HLTsubsequently defaults on its debt payments, the estimated value accepted a bad deal.
...
are well-behaved — there is no evidence of heteroscedasticity or undue influence fromlarge observations. Again, the Compressed APV methods perform at least as well as
...
regression that includes as independent variables the market-based Compressed APVvalues, the comparable company values, and comparable transaction values. All three
variables are statistically significant, the intercept term is not significantly different from0, and the variables together explain a bit more variation in transaction value than any
...
(6) Transaction Value Multiple = a + 13 Estimated Value Multiple + c
...
to 8.81% with 6% terminal value growth. As we noted earlier, we feel that a 4%growth rate is the economically most plausible assumption.
...
is used. The estimated market equity risk premium is remarkably close to the 7.42%median risk premium (calculated using the long-term arithmetic average of the spread
25
...
that equity returns are negatively related to firm size, positively related to the book-tomarket ratio and, controlling for firm size and book-to-market ratio, unrelated to equity
betas. Second, our results, reported in section 4, indicate that the Compressed APVmethod using market-based betas works about as well as industry-based betas. Both of
...
In our regressions, we exclude observations with negative pre-transaction book-tomarket ratios.
26
...
The patterns are qualitatively similar when one of the betas, size, and book-tomarket ratios are included in the same regression. In fact, the firm asset beta becomes
...
The previous sections indicate that the Compressed APVvaluation approaches
...
"fair," insiders and dealmakers may have an incentive to reduce their reported cash flowforecasts to justify the transaction value.
...
is statistically insignificant, while the second year shortfall is significant only atthe 10%level. The EBITDA and EBITDA margin shortfalls are also smaller than those
...
debt to transaction value). If hardwiring is causing our results, the CompressedAPVtechniques should be more accurate for the high debt sub-sample.
We do not find any significant difference in the performance of the
Compressed APV approaches for the two sub-samples. Using the market-based APVapproach, the estimated values are within 15% of transaction value for 56% of the lowdebt sub-sample and 65% of the high sub-sample. Also, the mean absolute error of the
...
there is an incentive to raise forecasts to get financing or get a higher price, but —unlike HLTs — there is no incentive to lower forecasts. If hardwiring and associated
...
value growth), the median APV estimate is 2.6% above the transaction value (mean is0.2%) when there is outside pressure and 1.6% below the transaction value (mean is
...
Although the DCF approaches perform at least as well as the comparablebased approaches, we find that the comparable-based estimates add explanatory power
...
between the implied risk premiums and eithertransaction value, i.e., firm size, or bookto-market ratios. For this sample, therefore, the results favor CAPM-based approaches
to discount rates over those based on size or book-to-market ratios.
35
References
Anders, George, 1992, Merchants ofDebt. New York: Basic Books.
...
Harper and Row.
Cornell, Bradford and Kevin Green, 1991, The investment performance of lowgrade bond funds, Journal of Finance 46, 29-48.
...
structure, and corporate performance: A study of leveragedrecapitalizations, Journal of Accounting and Economics 16, 209-23 6.
...
1984 2 0 2 0 0 0
1985 3 —
3 2 2 1 1 1986 8 8 4 4 4 4
1987 6_ 3 5 2 1 1988 24 14 22 12 2 2
1989 7 5 7 5 0 0
...
with industry asset betas from value-weighted industry pnsdobOs; (C) CAPM-based approach with market asset betas. Comparable values are
...
(median) 0.85 0.88 0.93 Measures for Valuation Errors:
...
Table 3
Sensitivity of CA?M-based approaches to equity risk premium and terminal value growth rate
Sensitivity of CAPM-bascd valuation methods to equity risk premium and terminal value growth rate assumptions In SI highly leveraged transactions completed between 1983 and 1989. Mcdian Is the
median of the valuation errors (in percent). The valuation errors equal the natural log of estimated values relative to transaction values. Mcan absolute error is the mean absolutc error of the valuation
errors. CA PM-based values arc the estimated present values of projected capital cash flows. Discount rates equal the long-term Treaswy bond yield at thc time of the projections plus lhc equity risk
premium times the relevant asset beta. In the base case in panel I, terminal values arc grown at 4% and the equity risk premium Is the arithmetic average premium or the S&P 500 return over the longterm Treasury bond return from 1926 until the year before the transaction Is announced. The median risk premium in the base case Is 7.423'.. In panel It, values arc estimated using equity risk premiums
of 5%, 6%. and 9%. In panel Ill, values arc estimated using terminal value growth rates or 0%, 23'., 6%. and 8%. Estimated present values are calculated using (A) CAPM-bascd approach with firm
asset betas; (B) CAPM-bascd approach with industry asset betas from value-weighted industry portfolios; (C) CAPM-based approach with market asset betas. The transaction value equals (I) the market
value of the firm common stock; (2) plus the market value of firm preferred stock; plus (3) the value of the firm debt; pluS (4) transaction fees; less (5) firm cash balances and marketable securities,
all at the time of the transaction. Debt not repaid in the transaction is valued at book value; debt that is repaid, at the repayment value,
2% 7.42%
6% 7.42%
-7.6% -10.7% -11.8%
...
(A) (B) (C) (A) (B) (C)
Firm Beta Industry Beta Market Beta Firm Beta Industry Beta Market Beta
I: Base case
4% 7.42% ( 6.0'/. ( 3.2% 1.0% 20.0% 16.5% 16.7%
equity premium II: Vary
4% 5% 22.9% 22.7% 19.8% 28.83'. 27.2% 25.6%
4% 6% 15.9% 14.1% 12.4% 23W. 21.0% 19.5%
4% 9% - 3.3% - 6.3% - 9.2% 19.9% 16.7% 18.03'.
lit: Vary terminal value growth rate
0% 7.42% 18.3% 17.7% 18.4%
20.3% 15.6% 16.8%
26.2% 21.0% 19.9%
36.93'. 30.93'. . 28.0%
Table 4 Cross-sectiOnal relation ofestimated vaJucs to transaction values Regressions oftransaction values on estimated net present values in SI highly
leveraged transactions completed between 1983 and 1989. Regressions
using multiples include transaction values and estimated net present values as multiples of EBIThA in the year before the transaction. Estimated
net present values e calculated using (A) CAPM-b*Sed
approach with finn asset betas; (B) CAPM-based approach with indusfly asset betas from
value-weighted industsy portfolios; (C) CAPM-baSCd approach with market asset betas; (D) comparable company approach, and (E) comparable
transaction approach. All CAPM-btsed approaches use a tenninal value growth rate of 4%. Transaction
...
Regressions of logs RegicuiOi%S of levels —
I 2. 3. I —
5. 6.
...
Slope 0.97' (0.03] 0.35' (0.10] 0.50' (0.09] 036' [0.10] 0.55' (0.11) 0.35' [0.11) P.' — 0.97 R' —039 P.' — 039 —
D. Coinp. Company
Constant 0.55' [0 17J 128' (023) 4.51' (0.82)
Slope 0.94' [0.03] 028' (0.09] 0.43' (0.121 0.29' [0.11) 0.55' [0.11) 0.40' (0.11)
R'-096 R'-022 R'—034
E. Comp. Transaction Constant 0.21 (0.16] 039 (0.77] 1.40 (3.49)
Slope 0.97' (0.021 035' (0.11) 0.82' (036] 0.45 (0.31) 0.85' (0.42) 0.49 10.33)
R' * 0.97 — P.' — 0.98 P.' — 0.09 P.' —0.48 —
P.' — 0.08 P.' — 0 54 N ohs SI SI SI 51 51 51
...
completed between 1983 and 1989. Annual capital cash flows equal net income + depreciation + change in deferred taxes + amortization + (cash
...
(5) firm cash balances and marketable securities, all at the time of the transaction. Debt not repaid in the trmnsactio is valued at book value, debt
...
risk premium equals
the difference between the implied discount rate and the yield on long-term Treasury bonds from (Ibbotson Associates) as the
...
Inlesquart
Range
15.77 16.28 2.69 3.06 10.37 24.16 51 4%
6%
2%
16.77
14.85 —
...
Max. N
[erminal Value GrowthRate
Median Mean
7.74
Std. Des 3.19
lnterqUt
Range 2.95 0.97 17.98 51 2.24 19.25 SI
-0.20 16.72 51
-l 40 15.53 51