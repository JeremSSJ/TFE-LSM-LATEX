...
liées qu'à des parts d'OPCVM inscrits auprès de la
F.S.M.A. ou à des actifs appartenant aux catégories ouvertes
aux OPCVM de droit belge. Les actifs non conventionnels
...
L'assurance-vie de la branche 23 est un contrat à
...
En outre, selon le contrat, il est permis à l'assureur de
répercuter la taxe sur les comptes-titres (TCT) sur l'assuré \cite{monastuces2021tact, spf_tct_faq}.
\hl{cette dernière phrase vraiment pas là selon moi}
...
\subsubsection*{La taxe sur les opérations d'assurance} \colorbox{red}{keep}
...
déduits d'emblée du montant versé \cite{wikifin_a}. \colorbox{red}{keep}

\hl{pour les frais d'entrée qu'on utilise, on prend la moyenne pondérée
pour l année 2020 car c est ce qu on a trouvé comme données dispo}

%https://www.fsma.be/sites/default/files/media/files/2024-04/2024_etude_couts_pillier2-3.pdf
...
\cite{wikifin_a}. \colorbox{red}{keep}

\hl{pour les frais de gestion qu'on utilise, on prend la moyenne pondérée
pour l année 2020 car c est ce qu on a trouvé comme données dispo, et dans 
cette moyenne, les frais des fonds internes sont inclus, ---- nous supposons aussi que l éventuelle taxe sur les compte-
titres est dedans ?}
...
souscripteur lui-même comme bénéficiaire en cas de vie \colorbox{red}{keep}
\cite[art.~2; art.~145\textsuperscript{9}]{cir92_2025}.

\hl{peut être dire de manière plus explicite que entre 18 et 65 ans conditionne le fait 
d'avoir la réduction d'impot et qu on ne l'a plus après 65 ans et apparemment on peut continuer au dessus de 65 ans
mais sans réduction d'impot et donc on s arrête à 65 pour en profiter jusqu à la fin}
...
pour l'année 2026 \cite{cir92_circ2026}. \colorbox{red}{keep}

Par dérogation aux autres produits d'assurance-vie, les
contrats d'épargne-pension sont exemptés de la taxe sur les
opérations d'assurance de 2\,\% \colorbox{red}{keep} habituellement prélevée sur
...
consommées (taxes et chargements) \cite[art.~184 à 186]{cdtd1927}. \colorbox{red}{keep}
...
8\,\% a déjà été prélevée et ne sera plus appliquée. \colorbox{red}{keep}
...
\hl{Dans le modèle, l'économie fiscale générée par la réduction d'impôt (Art. 145/9 CIR 92)
 est comptabilisée en t+1, conformément au mécanisme procédural de l'impôt des personnes physiques
  belge : les dépenses de l'année t sont déclarées au cours de l'exercice d'imposition t+1 (Art. 353 CIR 92)
  , et le remboursement est effectué dans les deux mois suivant l'envoi de l'avertissement-extrait
   de rôle, lequel intervient pour la majorité des contribuables entre août et décembre de t+1 
   (SPF Finances, Être remboursé : quand et comment, fin.belgium.be). Cette hypothèse constitue
    une simplification conservative : dans les cas où l'AER est envoyé après janvier t+2, 
    le flux réel serait légèrement surestimé en t+1.}
...
souscripteur. En cas de vie, ce contrat n'est pas exigible avant cette âge. \colorbox{red}{keep}
...
L'économie fiscale maximale s'élève donc à 735~euros. \colorbox{red}{keep}
...
prélèvement est reporté au dixième anniversaire du contrat \cite[art.~184 à 186]{cdtd1927}. \colorbox{red}{keep}
...
des obligations ou encore des fonds \colorbox{red}{keep}. Les sommes déposées ne constituent 
...
certains allant jusqu'à supprimer les frais de garde et
 de gestion \cite{guide_epargne}. \colorbox{red}{keep}

\hl{et donc retiré sur le côté}
...
\begin{itemize}
  \item 0,12\,\% pour les obligations et bons d'État, ainsi
    ...
    (EEE), plafonné à 1\,300~euros par opération~; \colorbox{red}{keep}
...
des investisseurs opérant sur de faibles montants. \colorbox{red}{keep}
...
15\,000~euros \cite[page 9]{chambre2025pv_actifs}. \colorbox{red}{keep}

Les actifs financiers relevant de l'épargne-pension et de
l'épargne à long terme sont toutefois exclus de ce nouvel
impôt \cite[page 39]{chambre2025pv_actifs}. \colorbox{red}{keep}
...
compte (si belge) \cite[art.~21]{cdtd1927}. \colorbox{red}{keep}

\hl{c est passé à 0.3 ?}

Les actions, obligations, fonds,
...
net à l'échéance. \colorbox{red}{keep}

\hl{dire quelque part qu on se base sur un etf accumulant, et qu on en prend bien diversifié 
comme ça c est assez général et accumulant car intérêts composés et on prend le TER de cet etf
IE00BYX2JD69 et du coup il a un TER de 0.2, et pour la tob rien ne dit que 0.12\% mais c est
bien ça peut être vérifier qu il rentre dans la catégorie de ce qu on a défini avant}
...
\end{equation} \colorbox{red}{keep}
...
on considère que chaque versement intervient en fin de période. \colorbox{red}{keep}
...
annexe est porté à la valeur qu'il aurait s'il était réinvesti jusqu'à l'échéance. \colorbox{red}{keep}
...
nombre d'années restantes entre ce flux et l'échéance \colorbox{red}{keep}. Ainsi, une économie
...
\subsubsection*{Construction pas à pas du capital final net à l'échéance} \colorbox{red}{keep}
...
réserve. Trois types de coûts sont comptabilisés séparément comme des flux
sortants~: les frais de courtage et la TOB, prélevés à chaque versement, et les
frais de gestion du courtier, prélevés chaque année sur la réserve~:
\begin{align}
  \text{Courtage}_t    &= v \times f_e^{\text{CT}} \\
  \text{TOB}_t          &= v \times \tau_{\text{TOB}} \\
  \text{FraisGestion}_t &= R_t \times f_{\text{gest}}^{\text{CT}}
\end{align}
où $f_e^{\text{CT}}$ est le taux de frais de courtage, $\tau_{\text{TOB}}$ le
taux de la TOB et $f_{\text{gest}}^{\text{CT}}$ le taux de frais de gestion
annuels du courtier, appliqué sur la réserve en fin d'année~$t$.
...
gestion du courtier sont comptabilisés séparément~:
\begin{align}
  \widetilde{R}_0 &= v \notag \\
...
des coûts récurrents capitalisés à l'échéance~:
\begin{align}
  \text{CFN}_{\text{CT}} &=
  ...
  -
    \underbrace{%
      \sum_{t=0}^{n} \text{FraisGestion}_t \times \bigl(1 + r_{\text{OLO}}(n-t)\bigr)^{n-t}
    }_{\text{frais de gestion courtier capitalisés}}
\end{align}
...
global pour cette combinaison. \colorbox{red}{keep}
...
instruments domine sur l'intégralité de la plage. \colorbox{red}{keep}
...
soit douze heatmaps au total. \colorbox{red}{keep}
...
\begin{enumerate}[label=\alph*)]
  ...
  \item Méthode d'exonération PV simplifiée. L'exonération des
    ...
    réel.
\end{enumerate}\colorbox{red}{keep}
