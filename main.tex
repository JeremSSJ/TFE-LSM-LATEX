...
Les taux de rendement retenus sont des rendements espérés sur le long terme. Par 
simplification, nous appliquons les mêmes taux de manière uniforme à chaque véhicule.
En pratique, le rendement annuel d'un portefeuille varie d'une année à l'autre,
et dépend de l'allocation d'actifs choisie par l'investisseur.

\subsubsection*{Versements annuels constants.}

Nous supposons des versements annuels et identiques d'une année à l'autre. Un versement mensuel
aurait introduit une fréquence de capitalisation supplémentaire sans modifier la logique
de comparaison~; le versement annuel est par ailleurs cohérent avec le cadre légal des
contrats branche~23, dont les primes peuvent être uniques ou périodiques.

\subsubsection*{Grille de versements de 1~euro jusqu'au plafond légal.}

La simulation balaie des versements annuels entiers allant de 1~euro jusqu'aux plafonds
légaux au-delà desquels les réductions d'impôt cessent de s'appliquer pour les contrats
d'assurance-vie concernés. Partir de 1~euro n'est pas représentatif de la pratique, mais
cette granularité est nécessaire pour identifier les points de croisement entre les deux
véhicules. Si, par exemple, le compte-titres est plus avantageux en dessous d'un certain
montant de versement et que l'assurance-vie le devient au-delà, une simulation débutant
à un montant trop élevé ne permettrait pas de localiser ce seuil et fournirait donc une
information incomplète sur la zone de préférence de chaque instrument.

\subsubsection*{Inflation et évolution du revenu non modélisées.}

L'inflation et l'évolution du revenu de l'investisseur au fil du temps ne sont pas
modélisées. Les versements restent donc nominalement constants sur toute la durée de
la simulation.

\subsubsection*{Investissement intégral du versement annuel.}

Le montant versé chaque année est supposé intégralement investi dans l'actif,
indépendamment du cours de celui-ci au moment de l'achat. En pratique, le prix d'un
actif fluctue dans le temps, de sorte que le même montant permet d'acquérir un
nombre de parts variable d'une année à l'autre. Cette abstraction reste toutefois
acceptable dans la mesure où de nombreux courtiers proposent aujourd'hui l'achat de
fractions de parts, permettant d'investir n'importe quel montant sans contrainte liée
à la valeur d'une part entière.

\subsubsection*{Effort d'épargne constant jusqu'à l'échéance.} La simulation exclut tout rachat
anticipé et toute interruption des versements. Pour la branche~23, un rachat avant
le 60\textsuperscript{e} anniversaire entraîne une imposition à 33\,\% sur la valeur
de rachat, auxquels s'ajoutent des frais de sortie contractuels pouvant atteindre
5\,\%~; cette option est donc structurellement défavorable et il n'est pas utile de
la modéliser pour trancher la comparaison.

\subsubsection*{Horizon commun fixé à 65~ans.} Le régime fiscal de l'épargne-pension prévoit que
les réductions d'impôt cessent de s'appliquer aux versements effectués après le
65\textsuperscript{e} anniversaire de l'assuré, ce qui en fait le point d'arrêt
naturel pour ce véhicule. Par souci d'homogénéité, le même horizon est retenu pour
le compte-titres et l'épargne à long terme, qui n'imposent pas de borne supérieure
légale similaire mais sont ici alignés sur 65~ans. Cet horizon commun joue donc le
rôle de point de sortie unique à partir duquel tous les capitaux sont comparés.

\subsubsection*{Âge de début plafonné à 55~ans.} La simulation n'inclut que des profils dont les
versements débutent au plus tard à 55~ans, la borne précise correspondant au
31~décembre de la 54\textsuperscript{e} année de vie, soit le dernier jour avant le
55\textsuperscript{e} anniversaire. Au-delà de ce seuil, la législation reporte la
date de prélèvement de la taxe anticipative du 60\textsuperscript{e} anniversaire
au dixième anniversaire du contrat~; pour un souscripteur débutant à 56~ans, ce
report déplacerait la taxation et la disponibilité du capital à 66~ans, au-delà de
l'horizon de 65~ans, ce qui romprait l'homogénéité de la comparaison.

\subsubsection*{Portefeuille entièrement investi en actions via un ETF accumulant.} L'investisseur
est supposé allouer l'intégralité de son capital en actions. Afin d'assurer la
comparabilité entre les deux véhicules, les actifs sous-jacents retenus sont des ETF
à réplication indicielle de type accumulant, c'est-à-dire réinvestissant
automatiquement les revenus dans le fonds plutôt que de les distribuer. Ce choix
évite de devoir modéliser une fiscalité sur les distributions et des niveaux de
rendement en dividendes variables selon les produits. Il correspond par ailleurs à la
tendance dominante chez les investisseurs particuliers actuels et est représentatif
des fonds disponibles dans les contrats de branche~23, dont certains permettent
effectivement de loger des ETF actions à 100\,\% en capitalisation.

\subsubsection*{Abstraction du spread et des coûts d'exécution.} Lors de chaque transaction,
l'investisseur supporte en pratique un coût implicite appelé spread, c'est-à-dire
l'écart entre le prix auquel un vendeur est prêt à céder l'actif et le prix qu'un
acheteur est prêt à payer. Ce coût, difficile à quantifier de manière générique car
variable selon la liquidité de l'instrument et les conditions de marché, est ignoré
dans la simulation. De même, les variations de cours survenant entre la passation
d'un ordre et son exécution effective ne sont pas prises en compte. Seuls les frais
explicitement définis dans les scénarios sont intégrés au calcul.

\subsubsection*{Frais constants par scénario.} Les frais d'entrée et de gestion sont fixes dans un
scénario donné~; leur évolution dans le temps n'est pas modélisée.

\subsubsection*{Taxe sur les comptes-titres couverte par les frais de gestion de la branche~23.}
La taxe sur les comptes-titres susceptible d'être répercutée sur les assurés est
supposée incluse dans les frais de gestion moyens retenus pour la branche~23. Cette
hypothèse est raisonnable~: la taxe ne s'applique qu'au-delà d'un million d'euros par
compte détenu~; un assureur regroupant de nombreux assurés dans ses fonds internes
peut franchir ce seuil en tant que titulaire unique, mais le montant ainsi répercuté
par assuré individuel reste négligeable.

\subsubsection*{Fiscalité figée au moment de la rédaction.} La réglementation fiscale est celle en
vigueur au moment de la rédaction. Tout changement législatif futur n'est pas pris en
compte.

\subsubsection*{Comparaison en capital final net à l'échéance.} La comparaison est conduite en
termes de capital final net à l'échéance (CFN) plutôt qu'en capital brut. Ce choix
permet d'intégrer la valeur temporelle des flux annexes~: les économies fiscales
annuelles de la branche~23 et les coûts récurrents du compte-titres sont capitalisés
jusqu'à l'horizon commun au taux sans risque OLO. Le résultat dépend du référentiel
de taux retenu.

\subsubsection*{Absence de risque de contrepartie.} Les deux instruments sont supposés exempts de
tout risque de défaut ou de faillite de l'assureur ou du courtier.

\subsubsection*{Méthode d'exonération des plus-values simplifiée.} L'exonération des plus-values
pour le compte-titres est calculée de manière forfaitaire (montant de base augmenté
d'un cumul annuel plafonné), ce qui constitue une approximation du mécanisme réel.

\subsubsection*{Remboursement fiscal décalé d'un an.} La réduction d'impôt liée aux versements
effectués au cours d'une année est supposée perçue l'année suivante, conformément
au fonctionnement habituel de la déclaration fiscale belge. Pour les versements
effectués à l'année~$n$ (dernière année de l'horizon), le remboursement arrive donc
après l'horizon de comparaison~; il est ramené à la date de référence par
actualisation au taux OLO à un an.

\subsubsection*{Réduction d'impôt systématiquement déclarée.} La simulation suppose que
l'investisseur demande la réduction d'impôt chaque année sans exception dans sa
déclaration fiscale annuelle. En pratique, omettre de la déclarer une année ferait
perdre la réduction correspondante~; l'hypothèse retenue maximise donc l'avantage
fiscal de la branche~23.

\subsubsection*{Épargne à long terme~: plafond de déduction atteint chaque année.} Pour l'ELT,
la simulation suppose que le revenu professionnel de l'investisseur est suffisant
pour atteindre le plafond légal de 2\,450~euros chaque année. En réalité, le montant
déductible est plafonné selon une formule liée aux revenus professionnels~;
les investisseurs dont les revenus sont insuffisants pour atteindre ce plafond ne
sont pas couverts par la simulation.

\subsubsection*{Flux annexes traités comme certains et taux OLO fixes.} Les économies fiscales
annuelles de la branche~23 et les coûts périodiques du compte-titres capitalisés
jusqu'à l'échéance sont traités comme des flux certains, ce qui justifie l'emploi
d'un taux sans risque pour leur capitalisation. La courbe des OLO belges retenue est
celle observée à la date de rédaction~; elle est supposée constante pour l'ensemble
de la simulation et toute variation ultérieure des taux n'est pas prise en compte.

\subsubsection*{TOB au taux de 0,12\,\%.} La simulation applique la taxe sur les opérations de
bourse au taux de 0,12\,\%, applicable aux ETF inscrits auprès de la FSMA ou d'un
régulateur d'un État membre de l'Espace économique européen. Ce taux correspond à
l'instrument retenu, à savoir un ETF indiciel large coté sur un marché réglementé
de l'EEE.
