...
Il convient de relever qu'en l'absence de frais et de taxe sur les plus-values 
pour le compte-titres dans ce scénario, ce dernier part structurellement 
avantagé. La comparaison repose dès lors majoritairement sur le différentiel 
de frais.
...
\hl{donc pour épargne long terme, existence de coût (frais de l assureur + fiscalité)
qui n existe pas pour compte titres dans ce scenario}

\hl{et du coup suppositions dans ce scenario que si elt gagne c est grace à économie fiscale
car comme elt a trop de désavantages intrinsèques, 
si il gagne c est forcément grâce à l avantage fiscal +

les économies fiscales rapportent elle même de l argent car capitalisées
(donc peut être suffisant de juste dire éco fiscale)

}

\hl{là ou compte titre gagnent il peut y avoir plusieurs raison 

déjà majoritairement des couts qu ils ne subissent pas
donc intérêts composés meilleurs qui finissent par grandir plus vite 
que éco fiscales de l elt 

mais sur horizon court ça n a pas le temps de devenir meilleur et peut se faire
battre grace à éco fiscale (découle de la supposition d avant)

}

L'ELT l'emporte dans les situations où ces mécanismes n'ont pas le temps
ou le rendement suffisant pour jouer pleinement (à 0\,\% et 1\,\%).  
Les économies fiscales annuelles de l'ELT prennent alors
...
Du côté de l'ELT, une réserve encore
...
\hl{donc pas suffisamment handicapant pour compenser les désavantages des elt dont on a parlé}
